...
public class ActionRestContoller {
	...
	public ResponseEntity<RestResponse> find(@PathVariable String id) {
		...
		try {
			...
			if (action != null) {
				...
				ActionModel data = new ActionModel(action, allStatusCodes, allActionCodes, baseUrlString);
	...
	public ResponseEntity<RestResponse> findAll() {
		...
		try {
			...
			List<ActionModel> data = new ArrayList<>();

			List<StatusCode> allStatusCodes = statusCodeService.findAll();
			List<ActionCode> allActionCodes = actionCodeService.findAll();
			
			for (Action action : actions) {
				data.add(new ActionModel(action, allStatusCodes, allActionCodes, baseUrlString));
			}
